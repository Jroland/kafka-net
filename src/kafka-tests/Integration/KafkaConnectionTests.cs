...
namespace kafka_tests.Integration
{
    ...
    public class KafkaConnectionTests
    {
        ...
        public void Setup()
        {
            ...
            _conn = new KafkaConnection(new KafkaTcpSocket(options.KafkaServerUri.First()), options.ResponseTimeoutMs, options.Log);
        }
        ...
        public void EnsureMultipleAsyncRequestsCanReadResponses()
        {
            ...
            var singleResult = _conn.SendAsync(new MetadataRequest()).Result;
        ...
        public void EnsureDifferentTypesOfResponsesCanBeReadAsync()
        {
            ...
            var ensureTopic = _conn.SendAsync(new MetadataRequest { Topics = new List<string>(new[] { ExpectedTopic }) }).Result;

            Assert.That(ensureTopic.Count, Is.GreaterThan(0));
            Assert.That(ensureTopic.First().Topics.Count, Is.EqualTo(1));
            Assert.That(ensureTopic.First().Topics.First().Name == ExpectedTopic, Is.True, "ProduceRequest did not return expected topic.");
            

            var result1 = _conn.SendAsync(RequestFactory.CreateProduceRequest(ExpectedTopic, "test"));
            var result2 = _conn.SendAsync(new MetadataRequest());
            var result3 = _conn.SendAsync(RequestFactory.CreateOffsetRequest(ExpectedTopic));
            var result4 = _conn.SendAsync(RequestFactory.CreateFetchRequest(ExpectedTopic, 0));
            ...
            Assert.That(result2.Result.First().Topics.Any(x => x.Name == ExpectedTopic), Is.True, "MetadataRequest did not return expected topic.");
